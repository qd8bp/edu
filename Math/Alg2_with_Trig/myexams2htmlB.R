myexams2html <- function(file, n = 1L, nsamp = NULL, dir = ".", template = NULL,
  ...
  function(exm, dir, info)
  {
    ...
    for(i in 1:nt) {
      ...
      for(j in seq_along(exm)) {
        ...
        html_body <- c(html_body, paste0('<canvas class="js-paint',j,' paint-canvas" width="600" height="300"></canvas>'))
        html_body <- c(html_body, paste0("<script> const paintCanvas = document.querySelector( '.js-paint",j,"' );const context = paintCanvas.getContext( '2d' );context.lineCap = 'round';
let x = 0, y = 0;
